    ...
    for(let i = 0; i < 4; i++){
        while (true){
            ...
            for(let j = 0; j < ship[i].length; j++){
                ...
                else if(defaultState.gameBoard[randomRow + randomDirec*j][randomCol + Direc*j] == 'X'){
                    ifEnough = false;
                }
            }
            ...
            if (ifEnough){
                for(let j = 0; j < ship[i].length; j++){
                    defaultState.gameBoard[randomRow + randomDirec*j][randomCol + Direc*j] = 'X';
                }
...
export default function gameReducer(state, action) {
    ...
    if (action.type === "EnemyTurn"){


        let row = randomNum(0,9);
        let col = randomNum(0,9);

        while(state[row][col] === 'O' || state[row][col] === 'x'){
            row = randomNum(0,9);
            col = randomNum(0,9)
        }
